<?php
$pageTitle = 'NH Modders';
$pageMetaTitle = 'Nintendo Homebrew';
$pageDescription = 'Nintendo Homebrew Approved Hardmodders List.';
$pageUrl = 'https://nintendohomebrew.com/';
require __DIR__ . '/header.php';
?>
<div class="intro"><h2>NH-Approved Hardmodders</h2>
	<div class="divider"></div>
	<p>As always, do your own research and use at your own risk.</p>
	</div>
		<p>This page will list approved hardmodders and their contact details.</p>
		<p>Feel free to ask for photos and or feedback/reputation/vouches.</p>
		<p>To inquire about services and prices please see their websites as they answer a lot of questions you may have. If they don't have a site or you would like to talk directly to them after you've looked at their site, contact your person of choice.</p>
		<p>In our experience, these individuals have shown both skill and trustworthiness. Despite that, the Nintendo Homebrew Server is not responsible for any issues & you use these individuals' services at your own risk. If you have an issue that can't be resolved, please message NH Mod-Mail with proof of said issue so we can take appropriate action.</p>
		<ul>
				<li>pbanj<br>
				(USA only for send in services. Serves worldwide for exporting systems for those who'd like to have a system that's already in the USA worked on, but then sent to them outside the US. Or buy an item.)<br>
				Hardmodder<br>
				Consoles Serviced: Switch v1, v2, and Lite. Pretty much any home console. Any device you want to put a USB-C port in.<br>
				Services: Port repair/replacements, Hardware Mod installs, Part repair/replacements.<br>
				Contact:<br>
				Discord: @pbanj<br>
				Shop: <a href="https://shop.pbanjin.space">https://shop.pbanjin.space</a><br>
				Website: <a href="https://pbanjin.space/">https://pbanjin.space</a><br>
				Email: <a href="https://shop.pbanjin.space/contact-pb">Contact</a><br>
				</li>
				<br><br>
			
				<li>Lazr1026<br>
				(USA only)<br>
				Hardmodder<br>
				Consoles Serviced: Wii, Wii U, Switch<br>
				Contact:<br>
				Discord: @lazr1026<br>
				GBATemp: <a href="https://gbatemp.net/members/lazr1026.541964/">GBATemp Account/</a><br></li>
				<br><br>
			
				<li>Wurmatron<br>
				(U.S.A)<br>
				Seller<br>
				Consoles: Original Gameboy through 3DS<br>
				Contact:<br>
				Discord: @wurmatron<br>
				Shop: <a href="https://shop.wurmforge.com/?wt_coupon=nhuser">https://shop.wurmforge.com/</a><br>
				Links: <a href="https://wurmforge.com/">https://wurmforge.com/</a><br></li>
				<br><br>
			
				<li>Emerekt<br>
				(Canada only)<br>
				Hardmodder<br>
				Console serviced: Switch (all models), 3DS excluding top screens or hinge ribbons, can always ask about other consoles.<br>
				Services: Modchip install, component replacement/repair, port replacements on some consoles.<br>
				Contact:<br>
				Discord: @emerekt.wav<br></li>
				<br><br>
			
				<li>DefenderOfHyrule<br>
				(Based in Netherlands, services Europe & The UK)<br>
				Hardmodder<br>
				Consoles Serviced: Switch, 3DS, Gamecube<br>
				Contact:<br>
				Discord: @defenderofhyrule<br></li>
				<br><br>
			
				<li>RedOverlord<br>
				(Based in the Netherlands, serves Europe)<br>
				Hardmodder<br>
				Consoles Serviced: Xbox, Xbox 360, Switch, & HDMI Mod installs<br>
				Contact:<br>
				Discord: @redoverlord<br>
				Instagram: <a href="https://www.instagram.com/toinescustomconsoles/">toinescustomconsoles</a><br></li>
				<br><br>
			
				<li>Tecd<code>0</code>c<br>
				(Based in Germany, serves worldwide)<br>
				Hardmodder<br>
				Consoles Serviced: 3DS (Capture Cards/Repair), Wii<br>
				Contact:<br>
				Website: <a href="https://www.merki.net/?lang=en">https://www.merki.net/?lang=en</a><br>
				e-mail: info@merki.net<br>
				Discord: @tecd<code>0</code>c<br>
				Discord Server: <a href="https://discord.gg/ZnKXCRT">https://discord.gg/ZnKXCRT</a><br></li>
				<br><br>
			
			<!-- <li>StashBoy<br>
				(USA only)<br>
				Repairs<br>
				Consoles Serviced: Switch, Repairs/Failed Modchip Installs Only<br>
				Contact:<br>
				Discord: @stashboy<br></li> -->
</div>
</div>
<script src="script.js"></script>
</body>
</html>
